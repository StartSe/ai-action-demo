...
export function trelloConfigurado(): boolean {
  return Boolean(getConfig("TRELLO_API_KEY") && getConfig("TRELLO_API_TOKEN") && getConfig("TRELLO_BOARD_ID"));
}
...
export function trelloAutorizado(): boolean {
  return Boolean(getConfig("TRELLO_API_KEY") && getConfig("TRELLO_API_TOKEN"));
}
