...
/** Botão "Reiniciar quadro de exemplo": descarta o quadro em memória deste visitante e devolve o quadro já recriado. Sem efeito quando o Trello está conectado (não há quadro de exemplo a reiniciar). */
export async function POST() {
  const id = await visitanteId();
  if (trelloConfigurado()) {
    const quadro = await trello.obterQuadro();
