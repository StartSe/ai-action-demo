    ...
    request<YouTubeStatus>("/api/youtube")
      .then((data) => {
        ...
        if (result?.connected && data.connected)
          setSuccess(
            "YouTube conectado. Seus próximos vídeos serão importados pela API oficial.",
          );
      })
      .catch((e) => {
  ...
  return (
    <section className="youtube-settings">
      <div className="connection-card">
        <h3>
          <Icon name="youtube" /> YouTube oficial
        </h3>
        <p>
        ...
        <p className="muted small">
          A conta precisa ter permissão para editar o vídeo. Vídeos públicos de
          outros canais não são liberados por esta conexão.
        </p>
        {status?.connected && (
        ...
        {status?.configured && !status.connected && (
          <p className="muted small">
            Você será direcionado ao Google e voltará automaticamente ao Mapify.
            O Google exige uma permissão ampla para baixar legendas; o Mapify
            usa essa conexão somente para leitura.
          </p>
        )}
      </div>
      {status && (
        ...
        >
          ...
          {status.managed ? (
            ...
            <>
              <p>
                O responsável por esta instalação cadastra o Mapify no Google
              ...
              <ol>
                ...
                <li>
                  ...
                  do tipo <strong>Aplicativo da Web</strong> e cadastre esta URI
                  de redirecionamento:
                </li>
              </ol>
            </>
          ...
          {!status.managed && (
            ...
            >
              ...
              <label>
                ...
                <input
                  type="password"
                  required={!status.configured || clientId !== status.clientId}
                  value={secret}
              ...
              {status.connected && (
                <p className="muted small">
                  Alterar o cliente ou o segredo exigirá conectar a conta Google
                  novamente.
                </p>
              )}
