...
export class ChatGPTBridge {
  ...
  async run({
    ...
    await this.start();
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para gerar o mapa.");
    ...
    const r = await this.rpc<{ thread: { id: string } }>("thread/start", {
      ...
      environments: [],
      baseInstructions:
        "Você ajuda a organizar conhecimento em mapas mentais. Responda em português. Use somente as ferramentas explicitamente fornecidas. Não use terminal, arquivos, navegador ou outras capacidades do sistema.",
...
const globalChat = globalThis as typeof globalThis & {
  mapifyChatGPT?: ChatGPTBridge;
};
export function chatGPT() {
  return (globalChat.mapifyChatGPT ??= new ChatGPTBridge());
}
