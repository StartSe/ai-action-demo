...
export function Topbar({ marca, nome, area, status, erro, resumo, usuario, notificacoes, navegacao = NAVEGACAO }: { marca: string; nome: string; area: string; status: Status | null; erro?: boolean; resumo?: string; usuario?: UsuarioTopbar | null; notificacoes?: NotificacaoTopbar[]; navegacao?: ItemNavegacao[] }) {
  ...
  // No celular o chip disputa a barra com a marca e o botão de menu: a mesma frase sem o convite
  // ("· conectar") cabe, e o convite continua a um toque de distância, já que o chip inteiro é o link.
  const textoLongo = erro ? "Servidor indisponível" : !status ? "Verificando IA" : status.ai ? "IA conectada" : "Modo demonstração · conectar";
  const textoCurto = erro ? "Fora do ar" : !status ? "Verificando" : status.ai ? "IA conectada" : "Demonstração";
  const texto = (
    <>
      <span className="max-[420px]:hidden">{textoLongo}</span>
      <span className="hidden max-[420px]:inline">{textoCurto}</span>
    </>
  );
 ...
 * `children` cabe abaixo da descrição, para um estado vazio que oferece mais de um caminho de saída
 * (ex.: os três exemplos prontos do Precificador). */
export function Empty({ ilustracao, titulo, descricao, acao, onAcao, acaoSecundaria, children }: { ilustracao?: ReactNode; titulo: string; descricao: string; acao?: string; onAcao?: () => void; acaoSecundaria?: { rotulo: string; url: string }; children?: ReactNode }) {
  return (
    <div className="h-full min-h-[520px] max-md:min-h-[320px] flex flex-col items-center justify-center text-center text-muted p-10 max-md:px-4 max-md:py-7 border border-dashed border-line rounded-card">
      {ilustracao && <div className="text-accent mb-[18px]">{ilustracao}</div>}
      <h2 className="text-ink text-lg font-bold mb-1.5">{titulo}</h2>
      <p className="max-w-[420px]">{descricao}</p>
      ...
      {children && <div className="w-full max-w-[720px] text-left">{children}</div>}
    </div>
  );
}
    ...
    <div className="px-3.5 py-2.5 flex flex-col gap-1.5">
      {(titulo || chip) && (
        <div className="flex items-start justify-between gap-2">
          {titulo && (onAbrir ? <button type="button" className="font-bold text-left text-accent-ink cursor-pointer" onClick={onAbrir}>{titulo.render(linha)}</button> : <div className="font-bold">{titulo.render(linha)}</div>)}
 ...
 * Com `link`, a linha inteira leva ao destino — no desktop pelo clique na linha, no celular pelo cartão. */
