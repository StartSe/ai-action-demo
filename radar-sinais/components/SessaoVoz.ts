...
type IncomingSocketEvent =
  ...
  | { type: "audio"; audio_event: { event_id: number; audio_base_64: string } }
...
export class SessaoVoz {
  ...
  async iniciar(resultadoId: string) {
    ...
    try {
      ...
      const { signedUrl, contexto } = await pedidoJSON<{ signedUrl: string; contexto: string }>("/api/radar/voz", { method: "POST", body: JSON.stringify({ resultadoId }), signal: this.abort.signal });
      if (this.encerrada) return;
      const stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true, noiseSuppression: true, autoGainControl: true, channelCount: 1 } });
      if (this.encerrada) { stream.getTracks().forEach(t => t.stop()); return; }
      this.stream = stream;
      if (this.encerrada) return;
      await this.context.audioWorklet.addModule("/radar-audio-worklet.js");
      if (this.encerrada) return;
      this.input = this.context.createMediaStreamSource(stream);
      this.worklet = new AudioWorkletNode(this.context, "radar-audio");
