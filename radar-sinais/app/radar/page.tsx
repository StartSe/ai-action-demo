...
import { grafoParaJSON } from "@/components/Grafo";
...
import { linhaConfianca, resumoRadar } from "@/lib/sinais";
import type { CodigoErroIA, Meta } from "@/lib/ai";
import type { DadosRadar, Radar } from "@/lib/types";

import { ConteudoRadar, Proveniencia, type ResultadoDados } from "@/components/ResultadoRadar";
...
function RadarAtual({ cadastro }: { cadastro: CadastroRadar }) {
  ...
  useEffect(() => {
    const atualizar = (evento: Event) => {
      if ((evento as CustomEvent).detail.radarId !== cadastro.id) return;
      fetch(`/api/radar?ultimo=1&radarId=${cadastro.id}`).then(r => r.ok ? r.json() : null).then(r => { if (r) setResultado(r); }).catch(() => {});
    };
    window.addEventListener("radar-atualizado", atualizar);
    return () => window.removeEventListener("radar-atualizado", atualizar);
  }, [cadastro.id]);
  ...
  return (
    <>
        ...
        {real && exibido?.id && <DestaquesRadar radar={radar} radarId={cadastro.id} resultadoId={exibido.id} />}
        <Monitoramentos dados={dados} />
