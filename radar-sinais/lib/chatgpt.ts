...
export class ChatGPTBridge {
  ...
  private async receive(m: Message) {
    ...
    if (m.id !== undefined && m.method) {
      ...
      this.send({
        ...
        error: {
          code: -32601,
          message: "Esta operação não está habilitada no Radar de Sinais.",
        },
      });
      return;
    }
  ...
  async run({
    ...
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para analisar seus temas.");
    ...
    const r = await this.rpc<{ thread: { id: string } }>("thread/start", {
      ...
      environments: [],
      baseInstructions:
        "Você analisa sinais de mercado com as evidências fornecidas. Responda em português. Use somente as ferramentas explicitamente fornecidas. Não use terminal, arquivos, navegador ou outras capacidades do sistema.",
...
const globalChat = globalThis as typeof globalThis & {
  radarChatGPT?: ChatGPTBridge;
};
export function chatGPT() {
  return (globalChat.radarChatGPT ??= new ChatGPTBridge());
}
