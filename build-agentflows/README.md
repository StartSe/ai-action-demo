# Build Agentflows — v0.13.1

Versão 0.13.1: correção do carregamento do histórico com validação das respostas da API. Inclui upload por arrastar e soltar, revisão em modal amplo, credenciais compartilhadas de embeddings, menu Credenciais e paginação de execuções no servidor. Veja a [comparação de recursos e preparação dos serviços](KNOWLEDGE-FLOWISE.md). Consulte o [histórico de versões](CHANGELOG.md). Para publicar no Coolify, consulte [DEPLOY-COOLIFY.md](DEPLOY-COOLIFY.md) e use `docker-compose.coolify.yml`.
