...
export function ToolSelect({ value, used, onChange }: { value: string; used: string[]; onChange: (id: string) => void }) {
  const id = useId(), input = useRef<HTMLInputElement>(null);
  const [open, setOpen] = useState(false), [query, setQuery] = useState(""), [active, setActive] = useState(0);
  const selected = AGENT_TOOL_CATALOG.find((item) => item.id === value);
  const options = AGENT_TOOL_CATALOG.filter((item) => normalize(item.name).includes(normalize(query)) && (!used.includes(item.id) || item.id === value));
  ...
  return <div className="tool-select" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) { setOpen(false); setQuery(""); } }}>
    <div className="tool-select-input">
      <ToolLogo id={value} />
      <input ref={input} role="combobox" aria-label="Ferramenta" aria-expanded={open} aria-controls={id} aria-autocomplete="list" aria-activedescendant={open && options[active] ? `${id}-${active}` : undefined}
      ...
      <button type="button" aria-label={open ? "Fechar lista" : "Abrir lista"} onMouseDown={(event) => event.preventDefault()} onClick={() => { if (open) setOpen(false); else { input.current?.focus(); setOpen(true); } }}><Icon name="chevron" size={16} /></button>
    </div>
