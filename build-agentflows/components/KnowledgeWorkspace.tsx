                    ...
                    {step === 1 && (
                      <>
                        ...
                        <label>
                          ...
                          <small>
                            {config.embeddings.provider === "openai"
                              ? "Inclua /v1 no endereço, quando exigido pelo serviço."
                              : "O modelo precisa estar instalado no servidor Ollama."}
                          </small>
                        </label>
                        <label>
                          Chave de acesso
                        ...
                        <p className="knowledge-note">
                          Essa conexão é própria da base e independe do modelo
                          escolhido no Agente.
                        </p>
                      </>
                    )}
                    {step === 2 && (
                      <>
                        <label>
                          ...
                          <select
                            value={config.vectorStore.provider}
                            onChange={(e) =>
                              changeConfig({
                                ...config,
                                vectorStore: {
                          ...
                          >
                            <option value="local">
                              Local · persistido neste aplicativo
                            </option>
                        ...
                        {config.vectorStore.provider === "qdrant" ? (
                          <>
                            <label>
                              ...
                              <input
                                required
                                type="url"
                                value={config.vectorStore.url}
                                placeholder="https://seu-cluster.qdrant.io:6333"
                                onChange={(e) =>
                                  changeConfig({
                                    ...config,
                                    vectorStore: {
                            ...
                            <label>
                              ...
                              <input
                                type="password"
                                autoComplete="new-password"
                                value={config.vectorStore.apiKey || ""}
                                placeholder={
                                  config.vectorStore.configured
                                    ? "Credencial salva · preencha para substituir"
                                    : "Chave do Qdrant"
                                }
                                onChange={(e) =>
                            ...
                            <p className="knowledge-note">
                              O app cria uma coleção isolada para cada versão da
                              base e remove a anterior depois de concluir.
                            </p>
                          </>
                        ) : (
                          <div className="knowledge-choice-note">
                            ...
                            <div>
                              <strong>Sem serviço adicional</strong>
                              <p>
                                Os vetores ficam no armazenamento persistente do
                                app, junto da base. A consulta compara o
                                significado dos trechos por similaridade.
                              </p>
                            </div>
                          </div>
                        )}
                    ...
                    {step === 3 && (
                      <>
                        <label>
                          ...
                          <select
                            value={config.recordManager.provider}
                            onChange={(e) =>
                              changeConfig({
                                ...config,
                                recordManager: {
                                  provider: e.target.value as "none" | "sqlite",
                                },
                              })
                            }
                          >
                            <option value="sqlite">
                              SQLite · reaproveitar conteúdo já indexado
                            </option>
                            <option value="none">
                              Sem Record Manager · gerar novamente
                            </option>
                          </select>
                        </label>
                        <div className="knowledge-choice-note">
                          ...
                          <div>
                            <strong>
                              {config.recordManager.provider === "sqlite"
                                ? "Atualizações sem trabalho repetido"
                                : "Nova geração completa"}
                            </strong>
                            <p>
                              {config.recordManager.provider === "sqlite"
                                ? "Reaproveita embeddings de trechos iguais quando provedor, endereço e modelo permanecem os mesmos. Conteúdo alterado é recalculado."
                                : "Recalcula os embeddings a cada indexação, mesmo que os documentos não tenham mudado."}
                            </p>
                            <p>
                              A nova indexação substitui o conjunto anterior.
                              Trechos removidos deixam de aparecer nas
                              consultas.
                            </p>
                          </div>
                        </div>
              ...
              <section className="knowledge-index-bar">
                ...
                <button
                  ...
                  disabled={
                    locked ||
                    configDirty ||
                    (base.config.embeddings.provider === "openai" &&
