// Cliente MCP Streamable HTTP: inicialização, sessão e respostas JSON ou SSE.
...
function iniciar(conexao: ConexaoMCP): Promise<Sessao> {
  ...
  const promessa = (async () => {
    const { resultado, idSessao } = await enviar(conexao, "initialize", {
      protocolVersion: PROTOCOLOS[0], capabilities: {}, clientInfo: { name: "entrevista-ia", version: "0.1.0" },
    });
