...
export function resolve(especificador, contexto, proximo) {
  ...
  if (especificador.startsWith("@/")) {
    ...
    const arquivo =
      !path.extname(base) && existsSync(`${base}.ts`) ? `${base}.ts` : base;
  ...
  if (especificador.startsWith(".") && !path.extname(especificador)) {
    const base = contexto.parentURL
      ? path.dirname(fileURLToPath(contexto.parentURL))
      : RAIZ;
    for (const tentativa of [
      `${especificador}.ts`,
      `${especificador}/index.ts`,
    ]) {
      if (existsSync(path.resolve(base, tentativa)))
        return proximo(tentativa, contexto);
    }
  }
