...
export class ChatGPTBridge {
  ...
  private async receive(m: Message) {
    ...
    if (m.id !== undefined && m.method) {
      ...
      this.send({
        ...
        error: {
          code: -32601,
          message: "Esta operação não está habilitada no Predictive Harness.",
        },
      });
      return;
    }
  ...
  async usage() {
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para consultar os limites.");
  ...
  async run({
    ...
    await this.start();
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para conversar sobre os dados.");
    ...
    const r = await this.rpc<{ thread: { id: string } }>("thread/start", {
      ...
      environments: [],
      baseInstructions:
        "Você é um analista de dados que responde sobre planilhas. Responda em português. Use somente as ferramentas explicitamente fornecidas. Não use terminal, arquivos, navegador ou outras capacidades do sistema.",
...
const globalChat = globalThis as typeof globalThis & {
  harnessChatGPT?: ChatGPTBridge;
};
export function chatGPT() {
  return (globalChat.harnessChatGPT ??= new ChatGPTBridge());
}
