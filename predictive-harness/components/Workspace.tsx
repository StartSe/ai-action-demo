...
import { Historico, type Aba } from "./Historico";
...
export function Workspace({ inicial = "conversa" }: { inicial?: Aba }) {
  const params = useSearchParams();
  const [aba, setAba] = useState<Aba>(params.get("aba") === "conectores" ? "conectores" : inicial);
  ...
  const [interpretacaoModal, setInterpretacaoModal] = useState(false);
  const [livro, setLivro] = useState(false);
  const [gestaoPremissas, setGestaoPremissas] = useState(params.get("aba") === "premissas");
  const [renomeando, setRenomeando] = useState<SessaoConversa | null>(null);
  ...
  useEffect(() => { void request<Status>("/api/status").then(setStatus).catch(() => setStatus(null)); }, [aba]);
  useEffect(() => {
    if (!atual) return;
    const url = new URL(window.location.href);
    url.pathname = aba === "configuracoes" ? "/configuracoes" : "/";
    if (aba !== "configuracoes") url.hash = "";
  ...
  function editarNome(c: SessaoConversa) { setRenomeando(c); setNome(c.titulo); setErroNome(""); }
  async function salvarNome() {
    if (!renomeando) return;
    setAcaoHistorico(true); setErroNome("");
    try { await request("/api/conversas", "PATCH", { id: renomeando.id, titulo: nome }); await carregarHistorico(); setRenomeando(null); }
    catch(e) { setErroNome((e as Error).message); }
    finally { setAcaoHistorico(false); }
  }
  ...
  return <div className="app jev-app">
    <div className="chat-layout">
      <Historico aberto={historico} atual={atual?.id} conversas={conversas} disabled={bloqueado} aba={aba} onAba={setAba} onOpen={() => alternarHistorico(true)} onClose={() => alternarHistorico(false)} onNova={() => void nova().catch(() => {})} onAbrir={c => { setAba("conversa"); void abrir(c); }} onFixar={c => void fixar(c)} onExcluir={c => { setExcluir(c); setConfirmar("excluir"); }} onRenomear={editarNome} onErro={setErro} />
    ...
    {aba === "conversa" ? <main className={"analysis-workspace" + (!vozAtiva && (mostrarHarness || livro) ? " with-insight" : "")}>
        <section className="conversation-main" aria-label="Conversa com Jev">
          <header className="conversation-toolbar">
            <div className="toolbar-context"><button className="icon-button workspace-menu-toggle" aria-label={historico ? "Recolher menu lateral" : "Abrir menu lateral"} aria-expanded={historico} onClick={() => alternarHistorico(!historico)}><Icon name="sidebar" size={19} /></button><button className="current-conversation-title" title="Renomear conversa" disabled={bloqueado || !atual} onClick={() => atual && editarNome(atual)}>{atual?.titulo || "Conversa com Jev"}<Icon name="edit" size={13} /></button></div>
            <div className="conversation-actions">
              ...
              <button className={"secondary pequeno premises-trigger" + (livro ? " active" : "")} aria-expanded={livro} onClick={() => { if (vozAtiva) setGestaoPremissas(true); else setLivro(!livro); setMostrarHarness(false); }}><Icon name="book" size={16} /><span>Premissas</span></button>
              <Popover label="Opções da conversa" className="icon-button" disabled={bloqueado || !atual} trigger={<Icon name="more" size={18} />}><><button onClick={() => atual && editarNome(atual)}><Icon name="edit" size={16} />Renomear conversa</button><button disabled={!mensagens.length} onClick={() => { setConfirmar("limpar"); }}><Icon name="refresh" size={16} />Limpar mensagens</button><button className="danger-text" onClick={() => { setExcluir(atual); setConfirmar("excluir"); }}><Icon name="trash" size={16} />Excluir conversa</button></></Popover>
            </div>
          </header>
          {atual && !busy ? <Conversa key={atual.id} conversaId={atual.id} base={base} mensagens={mensagens} harnessPronto={!!status?.harness} conversaPronta={!!status?.ai} selecionada={selecionada} onSelecionar={setSelecionada} onVerDecisoes={id => { setSelecionada(id); if (vozAtiva) setInterpretacaoModal(true); else setMostrarHarness(true); setLivro(false); }} onMensagens={ms => { setMensagens(ms); void carregarHistorico().catch(() => {}); }} onBaseMudou={carregarBase} onBusy={setRespondendo} onVozAtiva={setVozAtiva} onConectores={() => setAba("conectores")} autoPergunta={params.get("exemplo") === "1" && base?.demo ? base.sugestoes[1]?.texto || null : null} semRolagem={params.get("captura") === "1"} /> : <div className="empty-state"><span className="spinner" /><p>Preparando a conversa…</p></div>}
        </section>
        {livro && !vozAtiva && <PremissasPanel base={base} produto={produto} onProduto={setProduto} onClose={() => setLivro(false)} onGerenciar={() => setGestaoPremissas(true)} />}
        {mostrarHarness && !vozAtiva && <aside className="insight-panel" aria-label="Interpretação da IA"><header><div><span className="eyebrow">INTERPRETAÇÃO DA IA</span><h2>Como cheguei aqui</h2></div><IconButton icon="close" label="Fechar interpretação" onClick={() => setMostrarHarness(false)} /></header><div className="rolagem"><ComoCheguei mensagem={mensagens.find(m => m.id === selecionada) || null} onRecalcular={recalcular} onLivro={() => setGestaoPremissas(true)} /></div></aside>}
    </main> : aba === "configuracoes" ? <main className="settings-scroll"><div className="page-menu"><IconButton icon="sidebar" label="Abrir menu lateral" onClick={() => alternarHistorico(!historico)} /></div><Configuracoes /></main> : <main className="knowledge-page"><div className="page-menu"><IconButton icon="sidebar" label="Abrir menu lateral" onClick={() => alternarHistorico(!historico)} /></div><div className="page-intro"><span className="eyebrow">SEU ESPAÇO DE TRABALHO</span><h1>Fontes de dados</h1><p>Escolha os arquivos que vão orientar sua próxima conversa.</p></div><Base {...propriedadesBase} modo="conectores" /></main>}
      </div>
    </div>
    {renomeando && <Modal title="Renomear conversa" onClose={() => !acaoHistorico && setRenomeando(null)}><form onSubmit={e => { e.preventDefault(); void salvarNome(); }}><label>Nome da conversa<input autoFocus aria-label="Nome da conversa" maxLength={100} value={nome} onChange={e => setNome(e.target.value)} onFocus={e => e.target.select()} /></label><ErrorBox error={erroNome} /><div className="dialog-actions"><button className="secondary" type="button" disabled={acaoHistorico} onClick={() => setRenomeando(null)}>Cancelar</button><button className="primary" disabled={acaoHistorico || !nome.trim()}>{acaoHistorico ? "Salvando…" : "Salvar nome"}</button></div></form></Modal>}
    {interpretacaoModal && <Modal title="Premissas e cálculos" wide onClose={() => setInterpretacaoModal(false)}><ComoCheguei mensagem={mensagens.find(m => m.id === selecionada) || null} onRecalcular={recalcular} onLivro={() => setGestaoPremissas(true)} /></Modal>}
