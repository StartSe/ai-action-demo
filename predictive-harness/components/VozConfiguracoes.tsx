...
export function VozConfiguracoes() {
  ...
  return <section className="cartao largo" id="voz"><header><h3><Icon name="mic" size={18} /> Conversa por voz · ElevenLabs</h3><span className={"chip " + (status?.conectado ? "ok" : "neutral")}>{status?.conectado ? "Conectada" : "Opcional"}</span></header>
    ...
    <p>Durante a conversa, o microfone transmite áudio ao vivo para a ElevenLabs e o texto das respostas é enviado para síntese. As perguntas e respostas ficam no histórico; o áudio não é salvo nesta instalação. Encerre a conversa para desligar o microfone.</p>
  </section>;
}
