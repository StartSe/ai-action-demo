...
export function Base({ base, jevDisponivel, produtoSelecionado, onProduto, onAtualizar }: { base: DadosBase | null; jevDisponivel: boolean; produtoSelecionado: string | null; onProduto: (nome: string) => void; onAtualizar: () => Promise<void> }) {
  ...
  const enviar = (arquivo: File) =>
    acao("envio", async () => {
      ...
      const r = await request<{ planilha: Planilha }>("/api/planilhas", "POST", form);
      setAberta(r.planilha.id);
    });
  ...
  const remover = (p: Planilha) => (window.confirm(`Remover "${p.nome}"?`) ? acao("remover", () => request(`/api/planilhas/${p.id}`, "DELETE")) : undefined);
  const salvarPremissa = (produto: string, chave: ChavePremissa, valor: string) => acao(`premissa_${chave}`, () => request("/api/base/premissas", "PUT", { produto, chave, valor })).then(() => setEditando(null));
      ...
      <section className="bloco">
        ...
        <div className="lista-planilhas">
          {base.planilhas.map((p) => {
            ...
            const naBase = [base.matriculas, base.custos, base.marketing].includes(p.id);
            return (
              <div key={p.id} className={"planilha-item" + (abertaAqui ? " ativa" : "") + (naBase ? "" : " fora")}>
                <button className="planilha-cabecalho" onClick={() => setAberta(abertaAqui ? null : p.id)} aria-expanded={abertaAqui}>
                ...
                {abertaAqui && (
                  <div className="planilha-detalhe">
                    ...
                    <div className="perfil-acoes">
                      ...
                      {!p.demo && (
                        <button className="text-button" style={{ color: "var(--bad)" }} disabled={!!busy} onClick={() => void remover(p)}>
