// Leitura executiva da planilha (IA real ou fallback de demonstração), reaproveitada por app/api/insights/route.ts e lib/ferramentas.ts (MCP).
import { aiEnabled, askJSON, meta } from "./ai";
import { esperar, insightsDemo } from "./demo";
import type { Insights, Resumo } from "./types";
...
export async function gerarInsights(resumo: Resumo): Promise<{ demo: boolean; insights: Insights; meta: ReturnType<typeof meta> }> {
  const insumo = "totais e categorias da planilha enviada";
  if (!aiEnabled()) {
    await esperar(1100);
    return { demo: true, insights: insightsDemo(resumo), meta: meta({ demo: true, insumo }) };
  }
  ...
  return { demo: false, insights, meta: meta({ demo: false, insumo }) };
}
