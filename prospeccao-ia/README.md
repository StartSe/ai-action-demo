...
## Lista de leads — versão 0.7.0

A lista destaca nome, avatar, cargo, empresa e localização. Origem da prospecção ocupa no máximo duas linhas, com o título completo disponível ao passar o mouse. Busca sem acentos, filtros por prospecção/aderência/status, ordenação por prioridade/nome/dados recentes e páginas de 20 pessoas ficam no endereço para recarga e navegação. No celular, os contatos usam cartões e os filtros complementares podem ser expandidos.

A seleção pode abranger a página e oferece CSV dos selecionados, além do CSV completo dos resultados filtrados (incluindo outras páginas). Contatos selecionados fora do filtro são contados separadamente. O atalho “Ver abordagem” depende da existência de uma mensagem salva. A lista atualiza ao voltar à janela ou a cada 15 segundos enquanto visível e oferece atualização manual; falhas preservam os dados já exibidos e apresentam nova tentativa.

## Pesquisa adaptativa e acompanhamento — versão 0.7.0
...
A descoberta B2B prioriza **Bright Data**: Search Engine e `search_dataset` (com `list_dataset_fields`) entram nas primeiras rodadas, de até duas ações em paralelo. Os perfis são conferidos antes de decidir se a descoberta foi suficiente. Uma busca focada só na empresa também tem prioridade quando precisa recuperar cobertura. ProspectHalo, Exa, Tavily e SearchAPI entram se faltarem candidatos/contexto ou houver falhas. A revisão do plano por IA preserva essa prioridade. A busca geral de empresas e sinais também começa pela Bright Data e usa as demais fontes quando a primeira não retorna resultados pertinentes.
...
Ao reencontrar a mesma pessoa no mesmo produto, o LinkedIn canônico identifica a ficha existente. Novos dados públicos preenchem lacunas e observações atuais podem atualizar nome, cargo, empresa, localização, foto e contexto profissional. Fontes, evidências e sinais são incorporados. Respostas incompletas não apagam campos e leituras antigas/fracas não substituem dados mais recentes/consistentes. Status, papel manual, CRM, hipóteses e abordagens salvas são preservados. A pesquisa aprofundada da ficha também incorpora dados do perfil, e o contexto coletado fica disponível para novas mensagens. Mudança de cargo ou empresa desfaz a aderência calculada com os dados anteriores. Quando a empresa muda, a conta fica associada à prospecção original, sem referência dependente da busca nova. **Contatos já encontrados** aponta para essas fichas atualizadas. URLs regionais e variantes de idioma do LinkedIn são unificadas.
