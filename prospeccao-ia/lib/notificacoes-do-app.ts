// Como ESTE app avisa quem cuida das vendas (canal e destino escolhidos em /setup#notificacoes) e se já
// consegue entregar. Próprio do app: lib/notificacoes.ts (compartilhado) só envia; quem decide "está
// pronto?" é o app, porque a resposta depende do que ele promete na tela (aqui: os leads novos da
// semana com a abordagem de cada um já escrita).
...
export function canalDeAviso(): CanalDeAviso {
  ...
  if (canal === "email" && !destino) {
    return { canal, destino, motivo: "Informe o e-mail de destino em Notificações para receber os leads novos da semana." };
  }
