...
import { atualizarLead, atualizarProspeccao, criarConta, criarLead, listarContas, listarLeads, obterICP, obterProduto, obterProspeccao } from "./workspace";
...
function etapaEncontrarPessoas(prospeccaoId: string, criterios: Record<string, unknown>): void {
  ...
  function criarPessoa(contaId: string | null, empresa: string | null) {
    criarLead({
      prospeccaoId, contaId, nome: nomePessoaFicticia(indice),
      cargo: cargoCriterio || CARGOS_PESSOA[indice % CARGOS_PESSOA.length],
      empresa, cidade: cidadeCriterio || null, linkedin: null, fonte: null,
...
async function executarPipeline(prospeccaoId: string): Promise<void> {
  ...
  try {
    ...
    if (deveCriarPessoas(prospeccao.modo)) etapaEncontrarPessoas(prospeccaoId, prospeccao.criterios);
