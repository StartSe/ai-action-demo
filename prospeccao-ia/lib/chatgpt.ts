...
export class ChatGPTBridge {
  ...
  private async receive(m: Message) {
    ...
    if (m.id !== undefined && m.method) {
      ...
      this.send({
        ...
        error: {
          code: -32601,
          message: "Esta operação não está habilitada na Prospecção com IA.",
        },
      });
      return;
    }
  ...
  async beginLogin(): Promise<DeviceLogin> {
    await this.start();
    if (this.turns.size)
      throw new Error("Aguarde as gerações em andamento terminarem antes de trocar a conta.");
  ...
  async run({
    ...
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para qualificar leads e escrever abordagens.");
    ...
    const r = await this.rpc<{ thread: { id: string } }>("thread/start", {
      ...
      environments: [],
      baseInstructions:
        "Você apoia a prospecção comercial: qualifica leads e escreve estratégias e mensagens a partir das evidências fornecidas. Responda em português. Use somente as ferramentas explicitamente fornecidas. Não use terminal, arquivos, navegador ou outras capacidades do sistema.",
...
const globalChat = globalThis as typeof globalThis & {
  prospeccaoChatGPT?: ChatGPTBridge;
};
export function chatGPT() {
  return (globalChat.prospeccaoChatGPT ??= new ChatGPTBridge());
}
