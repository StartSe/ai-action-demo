...
// `useSearchParams`, que obrigaria a embrulhar a página num `Suspense`. Os passos 2 a 4 (US-010 a
// US-012) ainda não existem: avançar além do passo 1 mostra um resumo do que foi escolhido e um
...
export function ProspeccaoNova() {
  ...
  const podeContinuar = Boolean(produtoSelecionado && icpEfetivo);
  ...
  return (
    <>
      ...
      <main className="max-w-[720px] mx-auto px-8 pt-7 pb-12 max-md:px-4 max-md:pt-5 max-md:pb-10">
        ...
        <p className="apoio mb-6">Passo {passo} de {TOTAL_PASSOS} · Produto e perfil ideal</p>
        ...
        {passo <= ULTIMO_PASSO_PRONTO && (
          <div className="mt-6">
            <button type="button" className="btn-primary !w-auto max-md:!w-full" disabled={!podeContinuar} onClick={() => irParaPasso(2)}>Continuar</button>
          </div>
        )}
      </main>
    </>
