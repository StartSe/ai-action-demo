    ...
    try {
      const r = await fetch("/api/rotinas", {
        ...
        body: JSON.stringify({
          ...
          parametros: tipo === "cobranca-acoes" ? { janelaDias: janela } : undefined,
        }),
      });
      const d = await r.json();
      if (!r.ok) {
        ...
        {tipos.length === 0 ? (
          ...
          <form onSubmit={criar} className="flex flex-col gap-3">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              {tipo === "cobranca-acoes" && (
                <label className="flex flex-col gap-1 text-[13px] font-semibold w-40">
                  Dias antes do prazo
                  <input type="number" min={0} max={30} className="input" value={janela} onChange={(e) => setJanela(Number(e.target.value))} required />
                </label>
              )}
