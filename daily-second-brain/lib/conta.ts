...
export function criarConta(dados: {
  ...
  if (existeConta())
    throw new ErroConta(
      "Este app já tem uma conta. Entre com o seu e-mail e senha.",
      409,
    );
  ...
  ).run(nome, email, gerarHash(dados.senha));
  const linha = d
    .prepare("SELECT id, nome, email FROM usuarios WHERE email = ?")
    .get(email) as Usuario;
  return linha;
}
...
export function entrar(dados: { email: string; senha: string }): {
  ...
  if (
    bloqueio &&
    bloqueio.falhas >= TENTATIVAS_ATE_ESPERAR &&
    Date.now() < bloqueio.ate
  ) {
    throw new ErroConta(
  ...
  const linha = d
    ...
    .get(email) as (Usuario & { senha_hash: string }) | undefined;
...
export function trocarSenha(senha: string): boolean {
  ...
  const d = banco();
  const linha = d.prepare("SELECT id FROM usuarios LIMIT 1").get() as
    { id: number } | undefined;
  if (!linha) return false;
  d.prepare("UPDATE usuarios SET senha_hash = ? WHERE id = ?").run(
    gerarHash(senha),
    linha.id,
  );
...
export function aplicarRedefinicaoDeSenha(): void {
  ...
  try {
    if (trocarSenha(nova)) {
      console.warn(
        "NOVA_SENHA_ADMIN: senha da conta redefinida e sessões encerradas. Remova a variável depois de entrar.",
      );
    } else {
      console.warn(
        "NOVA_SENHA_ADMIN: nenhuma conta criada ainda; nada a redefinir.",
      );
    }
  } catch (err) {
    console.error(
      "NOVA_SENHA_ADMIN recusada:",
...
function abrirSessao(usuarioId: number): string {
  ...
  const expira = new Date(
    Date.now() + DIAS_DE_SESSAO * 86_400_000,
  ).toISOString();
  banco()
    .prepare(
...
export function usuarioDoToken(token: string): Usuario | null {
  ...
  banco()
    .prepare("UPDATE sessoes SET expira_em = ? WHERE token_hash = ?")
    .run(expira, hashDoToken(token));
