...
test("valida estrutura do modelo e recusa adaptações que alteram a metodologia", () => {
  ...
  const errado = structuredClone(QUESTIONARIO_MODELO);
  errado.perguntas[0].tipo = "texto";
  ...
  assert.equal(
    validarQuestionario({ titulo: "x", perguntas: [null], dimensoes: [] }),
    false,
  );
});
test("grupos antigos continuam válidos; área exige nome e meta válida", () => {
  ...
  for (const participantes of [0, -1, 2.5, "12", 100001])
    assert.throws(() => lerContexto({ participantes }));
  assert.deepEqual(
    lerContexto({
      grupoTipo: "area",
      grupoNome: " Marketing ",
...
test("participação usa a meta, sem confundir teto com tamanho do grupo", () => {
  ...
  assert.equal(participacao(a), 70);
  assert.equal(estadoColeta(a), "em-coleta");
