...
export class ChatGPTBridge {
  ...
  private async receive(m: Message) {
    ...
    if (m.id !== undefined && m.method) {
      ...
      this.send({
        ...
        error: {
          code: -32601,
          message: "Esta operação não está habilitada no Bússola de IA.",
        },
      });
      return;
    }
  ...
  async beginLogin(): Promise<DeviceLogin> {
    await this.start();
    if (this.turns.size)
      throw new Error("Aguarde as análises terminarem antes de trocar a conta.");
  ...
  async run({
    ...
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para usar a IA.");
    ...
    const r = await this.rpc<{ thread: { id: string } }>("thread/start", {
      ...
      environments: [],
      baseInstructions:
        "Você ajuda gestores a criar questionários e interpretar assessments de maturidade em IA usando apenas os dados fornecidos. Responda em português. Use somente as ferramentas explicitamente fornecidas. Não use terminal, arquivos, navegador ou outras capacidades do sistema.",
...
const globalChat = globalThis as typeof globalThis & {
  bussolaChatGPT?: ChatGPTBridge;
};
export function chatGPT() {
  return (globalChat.bussolaChatGPT ??= new ChatGPTBridge());
}
