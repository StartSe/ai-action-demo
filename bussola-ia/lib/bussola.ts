...
import {
  calcularDispersao,
  calcularMediasPorArea,
  calcularMediasPorDimensao,
  calcularNivelGeral,
  leituraSemIA,
  respostasTextoPorPergunta,
} from "./analise-bussola";
...
export const INSUMO_EXEMPLO =
  "8 respostas fictícias; para um diagnóstico real, crie o link de avaliação";

export async function gerarAvaliacaoExemplo(
  dados: DadosAvaliacao,
): Promise<{ avaliacao: Avaliacao; meta: Meta; id?: string }> {
  await esperar(900);
...
export async function gerarQuestionarioParaSetor({
  setor,
  porte,
  objetivo,
  grupoNome,
  ...
  if (!(await aiEnabled())) {
    ...
    if (objetivo?.trim()) {
      const pergunta = questionario.perguntas.find(
        (p) => p.tipo === "texto" && p.dimensao === "Resultados",
      );
      if (pergunta)
        pergunta.texto = `Pensando no objetivo “${objetivo.trim()}”, qual resultado concreto de inovação com IA você já observou${grupoNome ? ` na área ${grupoNome}` : " na empresa"}?`;
    }
    return {
      questionario,
      meta: meta({
        demo: true,
        insumo: `questionário modelo adaptado para o setor ${s}`,
      }),
    };
  }
  ...
  const questionario = await askJSON<Questionario>({
    system: SYSTEM_QUESTIONARIO,
    prompt,
  });
  if (!validarAdaptacao(questionario, QUESTIONARIO_MODELO))
    throw new ErroIA(
      "resposta_invalida",
      "O Arquiteto retornou uma estrutura incompleta. Tente novamente ou use o modelo revisado.",
      502,
    );
  return {
    questionario,
    meta: meta({
      demo: false,
      insumo: `questionário gerado para o setor ${s}${porte?.trim() ? `, porte ${porte.trim()}` : ""}`,
    }),
  };
}
...
type RespostaAnaliseIA = {
  resumo: string;
  leituraPorDimensao: LeituraDimensao[];
  forcas: string[];
  lacunas: string[];
  proximosPassos: string[];
  ondeDiscordam?: string[] | null;
};
...
export async function analisarAvaliacao({
  empresa,
  titulo,
  questionario,
  respostas,
  ...
  if (!respostas.length)
    throw new Error("Ainda não há respostas para analisar.");
  ...
  if (!(await aiEnabled())) {
    ...
    const entrada = {
      ...
      dispersao,
      mediasPorDimensao,
      ...(mediasPorArea.length >= 2 ? { mediasPorArea } : {}),
      respostasTexto,
    };
    try {
      extra = await askJSON<RespostaAnaliseIA>({
        system: SYSTEM_ANALISE,
        prompt: JSON.stringify(entrada),
      });
      const textos = (v: unknown): v is string[] =>
        ...
        v.every(
          (s) =>
            typeof s === "string" && s.trim().length > 0 && s.length < 4000,
        );
      if (
        !extra ||
        typeof extra.resumo !== "string" ||
        !extra.resumo.trim() ||
        !textos(extra.forcas) ||
        !textos(extra.lacunas) ||
        !textos(extra.proximosPassos) ||
        !Array.isArray(extra.leituraPorDimensao) ||
        extra.leituraPorDimensao.length !== mediasPorDimensao.length ||
        !mediasPorDimensao.every((m) =>
          extra.leituraPorDimensao.some(
            (l) =>
              l &&
              l.dimensao === m.dimensao &&
              typeof l.leitura === "string" &&
              l.leitura.trim(),
          ),
        ) ||
        (extra.ondeDiscordam != null &&
          (!Array.isArray(extra.ondeDiscordam) ||
            extra.ondeDiscordam.some((v) => typeof v !== "string")))
      )
        throw new ErroIA("resposta_invalida", "Leitura incompleta da IA.", 502);
    } catch (err) {
  ...
  const ondeDiscordam =
    mediasPorArea.length >= 2 ? (extra.ondeDiscordam ?? undefined) : undefined;
  ...
  const analise: Analise = {
    ...
    mediasPorDimensao,
    dispersao,
    leituraPorDimensao: extra.leituraPorDimensao,
    forcas: extra.forcas,
    lacunas: extra.lacunas,
    proximosPassos: extra.proximosPassos,
    ondeDiscordam,
    origemLeitura,
    avisoIA,
  };
  analise.conselho = await consultarConselho(
    analise,
    respostas.length,
    contexto,
  );
...
export function contextoDoLink(
  codigo: string,
): {
  empresa: string;
  titulo: string;
  ...
  const { questionarioId, empresa, titulo } = formulario.parametros as {
  ...
  if (!salvo)
    throw new Error(
      "O questionário desta avaliação não foi encontrado. Ele pode ter sido apagado em 'Meus questionários'.",
    );
  const p = formulario.parametros as unknown as ContextoAssessment;
  const contexto = {
    grupoTipo: p.grupoTipo ?? "empresa",
  ...
  return {
    empresa,
    titulo,
    questionario: salvo.questionario,
    respostas: listarPorCodigo(codigo),
    codigo,
    contexto,
  };
}
...
export async function analisarLink(
  codigo: string,
): Promise<{ avaliacao: Avaliacao; meta: Meta; id?: string } | null> {
