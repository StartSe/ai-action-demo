// Como ESTE app avisa quem criou a avaliação (canal e destino escolhidos em /setup#notificacoes) e se
// já consegue entregar. Próprio do app: lib/notificacoes.ts (compartilhado) só envia; quem decide "está
// pronto?" e "para quem?" é o app, porque a resposta depende do que ele promete na tela (aqui: o resumo
// diário da coleta de respostas).
...
export function canalDeAviso(): CanalDeAviso {
  ...
  if (canal === "email" && !destino) {
    return { canal, destino, motivo: "Informe o e-mail de destino em Notificações para receber o resumo da coleta." };
  }
