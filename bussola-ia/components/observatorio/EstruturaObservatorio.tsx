...
export function MarcaBussola() {
  return (
    <Link href="/" className="obs-brand" aria-label="Bússola · início">
      <span>
...
export function EstruturaObservatorio({
  ativo,
  status,
  totalAssessments,
  aoNavegar,
      ...
      <aside className="obs-sidebar">
          ...
          {nav.map((n) => {
            const conteudo = (
              <>
                <Icone nome={n.icone} />
                <span>{n.nome}</span>
                {n.id === "assessments" && totalAssessments !== undefined && (
                  <small>{totalAssessments}</small>
                )}
              </>
            );
            return aoNavegar ? (
              <button
                key={n.id}
                aria-current={ativo === n.id ? "page" : undefined}
                onClick={() => aoNavegar(n.id)}
              >
                {conteudo}
              </button>
            ) : (
        ...
        <div className="sidebar-bottom">
          ...
          <div className="obs-account">
            <span className="account-avatar">{nome[0]}</span>
            <div>
              <strong>{nome}</strong>
      ...
      <div className="obs-body">
        <header className="obs-topbar">
          <div>
            ...
            <span>/</span>
            <strong>
              {[...nav, ...utilidades].find((n) => n.id === ativo)?.nome}
            </strong>
          </div>
          <Link
            href="/setup"
