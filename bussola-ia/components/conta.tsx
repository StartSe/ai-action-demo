...
function EstruturaAcesso({ children }: { children: ReactNode }) {
  return (
    <main className="observatorio access-page">
      <section className="access-story" aria-label="Conheça a Bússola">
        ...
        <div className="access-story-content">
          ...
          <h2>
            Um novo olhar.
            <br />
            <em>Novos movimentos.</em>
          </h2>
          <p>
            Conheça o horizonte do seu time. Transforme as respostas de cada
            grupo em clareza para decidir o próximo passo.
          </p>
          <div className="access-orbit" aria-hidden="true">
            <span />
            <span />
            <span />
            <Icone nome="compass" size={105} />
            <i className="orbit-point first" />
            <i className="orbit-point second" />
            <i className="orbit-point third" />
          </div>
          <div className="access-pillars">
            <span>
              <Icone nome="people" size={16} /> Pessoas
            </span>
            <span>
              <Icone nome="spark" size={16} /> Inteligência
            </span>
            <span>
              <Icone nome="target" size={16} /> Ação
            </span>
          </div>
        </div>
        <p className="access-signature">
          BÚSSOLA <span>/</span> clareza para transformar.
        </p>
      </section>
      <section className="access-form-area" aria-label="Acesso ao painel">
        <div className="access-form">
          <p className="eyebrow">ESPAÇO DO GESTOR</p>
          {children}
          <p className="access-footnote">
            <Icone nome="shield" size={15} /> Um espaço para acompanhar a
            evolução do seu time.
          </p>
        </div>
      </section>
    </main>
...
function CaixaDeErro({ mensagem }: { mensagem: string }) {
  return (
    <div
      role="alert"
      className="mb-4 px-4 py-3 rounded-[10px] text-sm border bg-[#fde8e6] border-[#f5c2bd] text-danger"
    >
      {mensagem}
    </div>
  );
...
export function TelaCriarConta() {
  ...
  useEffect(() => {
    fetch("/api/conta")
      ...
      .then((r) => {
        ...
        if (r.existe)
          router.replace(
            `/entrar?next=${encodeURIComponent(proximoDestino())}`,
          );
      })
      .catch(() => {});
  }, [router]);
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!r.ok)
        throw new Error(resposta.error || "Não foi possível criar a conta.");
  ...
  return (
    <EstruturaAcesso>
      ...
      <p className="text-muted text-[13.5px] mb-5">
        Comece seu espaço de gestão. As conexões com IA e outros serviços são
        configuradas por você.
      </p>
      ...
      <form onSubmit={onSubmit} noValidate>
        <Field label="Seu nome" htmlFor="nome" hint="Aparece só na tela.">
          ...
          {erros.nome && (
            <p
              role="alert"
              id="erro-nome"
              className="text-[12.5px] text-danger mt-1"
            >
              {erros.nome}
            </p>
          )}
        ...
        <Field
          label="E-mail"
          htmlFor="email"
          hint="Vai ser usado para entrar no app."
        >
          <input
            id="email"
            ...
            <div className="flex items-center gap-2 mt-1.5">
              <div className="flex gap-1.5 flex-1" aria-hidden="true">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className={`h-1 flex-1 rounded-full ${i < forca.nivel ? (forca.nivel <= 2 ? "bg-warn" : "bg-ok") : "bg-line"}`}
                  />
                ))}
              </div>
        ...
        <Field label="Confirmar senha" htmlFor="confirmarSenha">
          <input
            ...
            aria-describedby={
              erros.confirmarSenha ? "erro-confirmarSenha" : undefined
            }
            type="password"
            className="input"
        ...
        <button type="submit" className="btn-primary mt-1" disabled={enviando}>
      ...
      {avisoDisco && (
        <p className="text-muted text-[12px] mt-4">
          Neste plano a conta e as configurações se perdem a cada nova
          publicação. Para manter, a equipe técnica adiciona um disco em Opções
          avançadas do Blueprint.
        </p>
      )}
    </EstruturaAcesso>
  );
...
export function TelaEntrar() {
  ...
  return (
    <EstruturaAcesso>
      <h1 className="access-title">Entrar</h1>
      <p className="access-intro">
        Bom ter você de volta. Seu próximo movimento começa aqui.
      </p>
