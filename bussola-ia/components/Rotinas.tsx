...
function descreverAgenda(r: Rotina): string {
  ...
  if (r.frequencia === "semanal")
    return `Toda ${DIAS_SEMANA[r.diaSemana ?? 0]} às ${r.hora}`;
  if (r.frequencia === "mensal")
    return `Dia ${r.diaMes ?? 1} de cada mês às ${r.hora}`;
  return r.dataUnica
    ? `Em ${data(r.dataUnica, { comAno: true })} às ${r.hora}`
    : `Uma vez às ${r.hora}`;
}
...
export function Rotinas() {
  ...
  function carregar() {
    ...
    requisitar<{
      ...
      enderecoPublicoDesconhecido?: boolean;
    }>("/api/rotinas")
      .then((d) => {
        setItens(d.itens);
        setTipos(d.tipos);
      ...
      .catch(() => setErro("Não foi possível carregar as rotinas."));
  }

  useEffect(() => {
    const t = setTimeout(carregar, 0);
    requisitar<StatusCodigo>("/api/rotinas/token")
      .then((d) => {
        setStatus(d);
        setEndereco(`${window.location.origin}/api/rotinas/executar`);
      })
      .catch(() => setErro("Não foi possível carregar o código das rotinas."));
  ...
  async function gerarCodigo() {
    ...
    setErro("");
    try {
      const d = await requisitar<{ codigo: string }>("/api/rotinas/token", {
        method: "POST",
      });
  ...
  async function pausar(id: string, ativa: boolean) {
    setErro("");
    try {
      await requisitar(`/api/rotinas/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  async function apagar(id: string) {
    if (
      !(await confirmar("Apagar esta rotina?", { confirmarRotulo: "Apagar" }))
    )
      return;
    setErro("");
    try {
      await requisitar(`/api/rotinas/${id}`, { method: "DELETE" });
      carregar();
    } catch (e) {
      setErro((e as Error).message);
    }
  ...
  async function executarAgora(id: string) {
    ...
    setErro("");
    try {
      await requisitar(`/api/rotinas/${id}/executar-agora`, { method: "POST" });
      carregar();
    } catch (e) {
      setErro((e as Error).message);
    } finally {
  ...
  const colunas: Coluna<Rotina>[] = [
    {
      ...
      titulo: "Rotina",
      papel: "titulo",
      render: (r) => tipos.find((t) => t.tipo === r.tipo)?.rotulo || r.tipo,
    },
    {
      chave: "agenda",
      titulo: "Quando",
      papel: "resumo",
      render: (r) => `${descreverAgenda(r)}${r.ativa ? "" : " · Pausada"}`,
    },
    {
      chave: "canal",
      titulo: "Canal",
      ...
      render: (r) =>
        r.ultimaFalha ? (
          <span className="text-danger">
            Falhou: {r.ultimaFalha} ·{" "}
            <a className="btn-link text-[12.5px]" href="/setup#notificacoes">
      ...
      render: (r) => (
        <div className="flex gap-3 flex-wrap">
          ...
          <button
            type="button"
            className="btn-link text-[12.5px]"
            onClick={() => pausar(r.id, !r.ativa)}
          >
            {r.ativa ? "Pausar" : "Retomar"}
          </button>
          <button
            type="button"
            className="btn-link text-[12.5px] text-danger"
            onClick={() => apagar(r.id)}
          >
            Apagar
          </button>
  ...
  return (
    <section id="rotinas" className="card p-6 max-md:p-5 settings-extra">
      <p className="eyebrow">03 / ACOMPANHAR</p>
      <h2 className="text-lg font-bold mb-1">Rotinas</h2>
      <p className="text-muted text-sm mb-4 max-w-[640px]">
        Agende o app para gerar e entregar um resultado sozinho, em um horário
        fixo, sem que ninguém precise abrir a tela.
      </p>

      {erro && (
        <div role="alert" className="obs-alert error">
          {erro}
          {erroMotivo === "notificacoes" && (
            <a className="text-link" href="/setup#notificacoes">
              Configurar notificações
            </a>
          )}
          {itens === null && (
            <button className="text-link" onClick={carregar}>
              Tentar novamente
            </button>
          )}
        </div>
      ...
      {enderecoDesconhecido && (
        <div className="mb-4">
          <Aviso tom="warn">
            Endereço público desconhecido: o aviso da próxima rotina sai sem o
            link do resultado. Abra o app pelo endereço publicado uma vez, ou
            informe em &quot;Para a equipe técnica&quot;.
          </Aviso>
        </div>
      )}
      ...
      <div className="mt-6 pt-5 border-t border-line">
        ...
        {tipos.length === 0 ? (
          <p className="text-muted text-sm">
            Nenhum tipo de rotina disponível neste app ainda.
          </p>
        ) : (
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <label className="flex flex-col gap-1 text-[13px] font-semibold flex-1 min-w-[200px]">
                ...
                <input
                  ...
                  placeholder={
                    canal === "email"
                      ? destinoPadrao || "voce@empresa.com"
                      : "#geral"
                  }
                  value={destino}
      ...
      <div className="mt-6 pt-5 border-t border-line">
        <MaisDetalhes titulo="Opções avançadas">
          <h3 className="text-sm font-semibold mb-2">
            Rodar sozinho, sem abrir o app
          </h3>
          <div className="mb-4">
            <Aviso tom="warn">
              No plano gratuito o app hiberna e a rotina só roda quando alguém
              acessa. Para rodar sozinho, use um plano pago ou chame esta URL de
              um agendador externo.
            </Aviso>
          </div>
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-3 flex-wrap">
              <span className="text-[13px] font-semibold w-[130px] shrink-0">
                Endereço
              </span>
              <code className="bg-bg border border-line px-2 py-1 rounded-md text-[12.5px] break-all flex-1 min-w-0 basis-[220px]">
                {endereco}
              </code>
            ...
            {codigoNovo ? (
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <code className="bg-bg border border-line px-2 py-1 rounded-md text-[12.5px]">
                  {status.mascarado}
                </code>
              </div>
            ) : null}
            ...
            {codigoNovo && (
              <p className="text-[12.5px] text-muted">
                Guarde este código agora: por segurança, ele não aparece de novo
                depois desta tela.
              </p>
            )}
          </div>
