...
export function CartaoIndicador({ dados, titulo }: { dados: DadosIndicador; titulo: string }) {
  // Sem `anterior` não há período anterior para comparar (planilha sem coluna de data): a linha de
  // variação some em vez de mostrar 0%.
  const comparavel = dados.anterior !== undefined;
  const v = variacao(dados.valor, dados.anterior ?? dados.valor);
  ...
  return (
    <div>
      ...
      {comparavel && dados.anterior !== undefined && (
      ...
      <p className="sr-only">
        {comparavel && dados.anterior !== undefined
          ? `${titulo}: ${formatar(dados.valor, dados.formato)}, ${variacaoTexto(v)} contra ${formatar(dados.anterior, dados.formato)} no período anterior.`
          : `${titulo}: ${formatar(dados.valor, dados.formato)}.`}
      </p>
    </div>
  );
}
