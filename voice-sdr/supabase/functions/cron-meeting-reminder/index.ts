// Adaptador Deno da rotina cron-meeting-reminder. Só amarração: lê o ambiente,
...
import { atenderRotina, type LembreteEnfileirado, type PortaDoLembrete } from './rotina.ts'
...
const porta: PortaDoLembrete = {
  async enfileirarLembretes(instante, limite) {
    const { data, error } = await servico.rpc('enfileirar_lembretes_de_reuniao', {
      p_agora: instante,
      p_limite: limite,
    })
    if (error) throw new Error(error.message)
    return (data ?? []) as LembreteEnfileirado[]
  },
}
