// Adaptador Deno da rotina cron-speed-to-lead. Só amarração: lê o ambiente,
// monta as portas sobre o Supabase e entrega a decisão para
// `enfileiramento.ts`.
//
...
//
// Nada aqui chama `call-place`: esta rotina só enfileira, e quem disca é
// `cron-dial` (L-14).
...
import { atenderRotina, type LeadCandidato, type PortaDoFalaRapido } from './enfileiramento.ts'
...
const porta: PortaDoFalaRapido = {
  async candidatos(instante, limite) {
    ...
    if (error) throw new Error(error.message)
    return (data ?? []) as LeadCandidato[]
  },

  // `ignoreDuplicates` é o `on conflict do nothing` no único de R-09
  // (`dial_queue_unica_por_fonte`): linha barrada volta vazia.
  async enfileirar(linha) {
    const { data, error } = await servico
      .from('dial_queue')
      .upsert(linha, { onConflict: 'account_id,source,source_ref,attempt', ignoreDuplicates: true })
      .select('id')
    if (error) throw new Error(error.message)
  ...
  async registrarIgnorado(linha) {
    const { error } = await servico
      .from('speed_to_lead_skips')
      .upsert(linha, { onConflict: 'lead_id', ignoreDuplicates: true })
    if (error) throw new Error(error.message)
  },
}
