...
// supabase/functions/cron-meeting-reminder/index.ts. Não edite à mão: rode `npm run pacote`.
//
...
//#endregion
//#region supabase/functions/cron-meeting-reminder/rotina.ts
const NOME_DA_ROTINA = "cron-meeting-reminder";
function lembrarReunioes(pedido) {
	const { porta } = pedido;
	return executarRotina({
		...
		trabalho: {
			async reivindicar(limite, instante) {
				return (await porta.enfileirarLembretes(instante, limite)).map((linha) => ({
					chave: linha.meeting_id,
...
async function atenderRotina(pedido, lembrete, opcoes) {
...
//#endregion
//#region supabase/functions/cron-meeting-reminder/index.ts
...
const porta = { async enfileirarLembretes(instante, limite) {
	const { data, error } = await servico.rpc("enfileirar_lembretes_de_reuniao", {
		p_agora: instante,
		p_limite: limite
	});
	if (error) throw new Error(error.message);
