...
// supabase/functions/tool-reschedule/index.ts. Não edite à mão: rode `npm run pacote`.
//
...
const FALAS_DO_RESGATE = { semInteresse: "Tranquilo, valeu por me contar. Se mudar de ideia, é só chamar a gente." };
//#endregion
...
//#endregion
//#region supabase/functions/tool-reschedule/remarcacao.ts
const REMARCAVEIS = new Set([
	"scheduled",
	"confirmed",
	"no_show"
]);
...
function recusa(motivo, speech, data = {}) {
	return {
		ok: false,
		data: {
			reason: motivo,
...
function motivoDe(valor) {
	return typeof valor === "string" ? valor.trim() : "";
}
function executorDaRemarcacao(leitura) {
	return {
		async ler(contexto) {
			...
			if (acao === "cancel") {
				...
				if (reuniao.status === "canceled") return {
					data,
					speech: FALAS_DO_LEMBRETE.cancelada
				};
				if (reuniao.status === "no_show") return {
					data: {
						meeting_id: reuniao.id,
						action: "cancel",
						status: "no_show"
					},
					speech: FALAS_DO_RESGATE.semInteresse
				};
				if (!CANCELAVEIS.has(reuniao.status)) return recusa("status_nao_elegivel", FALAS_DO_LEMBRETE.naoEstaMaisMarcada, { status: reuniao.status });
				return {
					data,
					speech: FALAS_DO_LEMBRETE.cancelada,
					plano: {
			...
			if (feita !== null) return {
				data: {
					meeting_id: feita.id,
					action: "reschedule",
					starts_at: feita.inicio
				},
				speech: falarRemarcacao({
					inicio: feita.inicio,
					fusoDoLead: reuniao.fusoDoLead,
					fusoDoEspecialista: reuniao.fusoDoEspecialista
				}, agora)
			};
			if (!REMARCAVEIS.has(reuniao.status)) return recusa("status_nao_elegivel", FALAS_DO_LEMBRETE.naoEstaMaisMarcada, { status: reuniao.status });
		...
		async efeitos(contexto, leitura) {
			...
			if (plano.tipo === "cancelar") {
				...
				if (resultado === "status_nao_elegivel") return {
					ok: false,
			...
			const escrita = await contexto.escrita.remarcarReuniao(contexto.contaId, contexto.chamada.id, plano.posicao, plano.motivo, new Date(contexto.agora()).toISOString());
			if (escrita.resultado === "remarcada" || escrita.resultado === "ja_remarcada") {
				...
				return {
					data: {
						meeting_id: escrita.meeting_id,
						action: "reschedule",
						starts_at: leitura.data?.starts_at ?? null
					},
					speech: leitura.speech
				};
			}
			if (escrita.resultado === "posicao_nao_oferecida" || escrita.resultado === "oferta_expirada") return {
				ok: false,
				data: { reason: escrita.resultado },
				speech: FALAS_DA_AGENDA.ofertaSemValidade,
				erro: escrita.resultado
			};
			if (escrita.resultado === "status_nao_elegivel") return {
				ok: false,
				data: { reason: escrita.resultado },
				speech: FALAS_DO_LEMBRETE.naoEstaMaisMarcada,
				erro: escrita.resultado
			...
			if (fala === void 0) throw new Error(`código desconhecido de remarcar_reuniao: ${escrita.resultado}`);
			return {
				ok: false,
				data: { reason: escrita.resultado },
				speech: fala,
...
function criarToolReschedule(leitura) {
...
//#endregion
//#region supabase/functions/tool-reschedule/index.ts
...
const leitura = {
	...
	async remarcacaoDaChamada(contaId, chamadaId, reuniaoId) {
		const { data, error } = await servico.from("meetings").select("id, starts_at").eq("account_id", contaId).eq("booked_call_id", chamadaId).not("rescheduled_from_id", "is", null).or(`rescheduled_from_id.eq.${reuniaoId},id.eq.${reuniaoId}`).limit(1).maybeSingle();
		falhou(error);
		const linha = data;
		return linha === null ? null : {
			id: linha.id,
			inicio: new Date(linha.starts_at).toISOString()
		};
	}
};
const escrita = {
