...
// supabase/functions/tool-confirm-meeting/index.ts. Não edite à mão: rode `npm run pacote`.
//
...
function falarPresencaConfirmada(reuniao, agora) {
...
//#endregion
//#region supabase/functions/tool-confirm-meeting/confirmacao.ts
...
function executorDaConfirmacao(leitura) {
	return {
		async ler(contexto) {
			...
			const data = {
				...
				status: "confirmed",
				starts_at: reuniao.inicio
			};
			if (reuniao.status === "confirmed") return {
				data,
				speech: falarPresencaConfirmada(reuniao, agora)
			};
			if (reuniao.status !== "scheduled") return recusa("status_nao_elegivel", FALAS_DO_LEMBRETE.naoEstaMaisMarcada, {
				meeting_id: reuniao.id,
			...
			return {
				data,
				speech: falarPresencaConfirmada(reuniao, agora),
				plano: { confirmar: true }
			};
		},
		async efeitos(contexto, leitura) {
			if (leitura.ok === false || leitura.plano === void 0) return;
			const resultado = await contexto.escrita.confirmarReuniao(contexto.contaId, contexto.chamada.id, new Date(contexto.agora()).toISOString());
			if (resultado === "confirmada" || resultado === "ja_confirmada") return;
			if (resultado === "status_nao_elegivel") return {
				ok: false,
				data: {
					reason: resultado,
					meeting_id: leitura.data?.meeting_id ?? null
				},
...
function criarToolConfirmMeeting(leitura) {
...
//#endregion
//#region supabase/functions/tool-confirm-meeting/index.ts
...
const leitura = { async reuniaoDaChamada(contaId, chamadaId) {
...
const escrita = { async confirmarReuniao(contaId, chamadaId, agora) {
