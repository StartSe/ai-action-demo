...
// supabase/functions/whatsapp-inbound/index.ts. Não edite à mão: rode `npm run pacote`.
//
...
function lerConversa(dado) {
	...
	return {
		...
		motivoDoFim: primeira && typeof primeira === "object" ? textoOuNulo$2(primeira.finish_reason) : null,
		modelo: textoOuNulo$2(corpo.model)
	};
}
function numero$1(valor) {
	return typeof valor === "number" && Number.isFinite(valor) ? valor : null;
}
function textoOuNulo$2(valor) {
...
const ENDPOINT$1 = "api/v1/chat/completions";
...
function corpoDaLeitura(pedido) {
	const parte = pedido.conteudo.tipo === "imagem" ? {
		type: "image_url",
		image_url: { url: pedido.conteudo.dataUri }
	} : {
		type: "input_audio",
		input_audio: {
			data: pedido.conteudo.base64,
			format: pedido.conteudo.formato
		}
	};
	return {
		model: pedido.modelo,
...
async function lerMidiaComModelo(contaId, resolvido, pedido, porta, opcoes = {}) {
	...
	try {
		const resposta = await buscar(URL_DA_CONVERSA, {
			...
			signal: AbortSignal.timeout(opcoes.limiteMs ?? 6e4)
		});
		let corpo = null;
		try {
			corpo = await resposta.json();
...
function lerMensagemRecebida(corpo) {
	const dado = objeto$1(corpo);
	if (dado === null) return {
		tipo: "ignorar",
		motivo: "nao_e_mensagem"
	};
	const tipo = texto$3(dado.type);
	if (tipo !== null && tipo !== "ReceivedCallback") return {
		tipo: "ignorar",
		motivo: "nao_e_mensagem"
	};
	if (dado.fromMe === true) return {
		tipo: "ignorar",
		motivo: "de_mim"
	};
	const bruto = texto$3(dado.phone) ?? "";
	if (dado.isGroup === true || dado.isNewsletter === true || dado.broadcast === true || /-group$|@g\.us$|@newsletter$/.test(bruto)) return {
		tipo: "ignorar",
		motivo: "grupo"
	};
	const id = texto$3(dado.messageId);
	if (id === null) return {
		tipo: "ignorar",
		motivo: "sem_id"
	};
	...
	const mensagem = texto$3(objeto$1(dado.text)?.message) ?? "";
	let midia = null;
	let legenda = "";
	let anexo = null;
	for (const [campo, nome, campoDaUrl] of MIDIAS) {
		const conteudo = objeto$1(dado[campo]);
		if (conteudo !== null) {
			midia = nome;
			legenda = texto$3(conteudo.caption) ?? "";
			const url = campoDaUrl === null ? null : texto$3(conteudo[campoDaUrl]);
			if (url !== null) anexo = {
				url,
				mime: texto$3(conteudo.mimeType)
			};
			break;
		}
	}
	if (midia === null && mensagem === "") {
		if (!Object.keys(dado).some((chave) => chave !== "text" && objeto$1(dado[chave]) !== null)) return {
			tipo: "ignorar",
			motivo: "vazia"
		};
		midia = "outro";
	...
	return {
		...
		idDoProvedor: id,
		nomeDoRemetente: texto$3(dado.senderName) ?? texto$3(dado.chatName),
...
const ESTADOS_DA_ENTREGA = new Map([
	["SENT", "enviada"],
	["RECEIVED", "entregue"],
	["DELIVERED", "entregue"],
	["READ", "lida"],
...
function lerStatusDeMensagens(corpo) {
	const dado = objeto$1(corpo);
	if (dado === null || texto$3(dado.type) !== "MessageStatusCallback") return null;
	const estado = ESTADOS_DA_ENTREGA.get((texto$3(dado.status) ?? "").toUpperCase());
	if (estado === void 0) return null;
	const ids = (Array.isArray(dado.ids) ? dado.ids : [dado.messageId, dado.id]).map(texto$3).filter((id) => id !== null);
	return ids.length === 0 ? null : {
		estado,
		ids
	};
}
function eStatusDeMensagem(corpo) {
	return texto$3(objeto$1(corpo)?.type) === "MessageStatusCallback";
}
//#endregion
...
//#endregion
//#region supabase/functions/_shared/whatsapp/descadastro.ts
...
function normalizarParaDescadastro(texto) {
	return texto.normalize("NFD").replace(/\p{M}/gu, "").toLowerCase().replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ").trim();
}
function ePedidoDeDescadastro(texto) {
...
async function contaDoEnderecoDoWhatsapp(endereco, chaves, agora) {
	if (chaves.vigente.trim() === "") return null;
	let conta;
	let chave;
	try {
		const url = new URL(endereco);
		conta = (url.searchParams.get("conta") ?? "").trim().toLowerCase();
		chave = (url.searchParams.get("chave") ?? "").trim();
	} catch {
		return null;
	}
...
//#endregion
//#region supabase/functions/whatsapp-inbound/respostas.ts
const MENSAGENS = {
...
function falhaInterna() {
	return {
		status: STATUS.falha_interna,
		corpo: {
			ok: false,
			motivo: "falha_interna",
			mensagem: MENSAGENS.falha_interna
		},
		depois: null
	};
}
...
function responderDepois(contaId, conversaId) {
	return (responder) => responder(contaId, conversaId);
}
async function receberWebhook(pedido, porta, ambiente) {
	const log = ambiente.log ?? logPadrao;
	if (pedido.metodo !== "POST") return {
		status: STATUS.metodo_invalido,
		corpo: {
			ok: false,
	...
	try {
		...
		if (existente !== null) {
			...
			return aceita({ duplicada: true }, await porta.canalLigado(contaId) ? responderDepois(contaId, existente.conversaId) : null);
		}
		...
		const midiaParaLer = antes !== null && eMidiaLida(mensagem.midia) && antes.status === "assistente" && await porta.canalLigado(contaId) ? mensagem.midia : null;
		...
		return aceita({ gravada: true }, (await porta.lerConversa(contaId, conversaId))?.status === "assistente" && await porta.canalLigado(contaId) ? responderDepois(contaId, conversaId) : null);
	} catch (erro) {
		log("falha_ao_gravar", {
			contaId,
...
function recusa$1(motivo, speech) {
	return {
		ok: false,
		data: { reason: motivo },
		speech,
...
function executorDoAgendamento(leitura) {
	return {
		async ler(contexto) {
			...
			if (modalidade === null) return recusa$1("modalidade_desconhecida", FALAS_DA_AGENDA.qualModalidade);
			const leadId = contexto.chamada.lead_id;
			if (leadId === null) return recusa$1("chamada_sem_lead", FALAS_DA_AGENDA.falha);
			...
			if (oferta === null) return recusa$1("posicao_nao_oferecida", FALAS_DA_AGENDA.ofertaSemValidade);
			if (Date.parse(oferta.expires_at) <= agoraMs) return recusa$1("oferta_expirada", FALAS_DA_AGENDA.ofertaSemValidade);
			...
			if (calendario !== null) {
				...
				if (conferencia.ok && !conferencia.valor.livre) return recusa$1("horario_ocupado_no_calendario", FALAS_DA_AGENDA.horarioTomado);
			}
			const horario = {
...
//#endregion
//#region supabase/functions/whatsapp-inbound/index.ts
...
const LIMITE_DA_LEITURA_DE_MIDIA_MS = 6e4;
const APLICACAO = {
...
const portaDaPergunta = { async chaveDoOpenRouter(conta) {
...
const porta = criarPortasDoCanal({
	...
	async rodada(contaId, pedido) {
		...
		}, portaDaPergunta, APLICACAO, LIMITE_DO_MODELO_MS);
	},
	buscarMidia: (url, init) => fetch(url, {
		...init,
		signal: AbortSignal.timeout(LIMITE_DO_DOWNLOAD_MS)
	}),
	async lerMidiaComModelo(contaId, tarefa, pedido) {
		...
		const resolvido = modeloDaTarefa((data ?? [])[0] ?? null, tarefa);
		return {
			...await lerMidiaComModelo(contaId, resolvido, {
				...pedido,
				modelo: resolvido.modelo
			}, portaDaPergunta, {
				aplicacao: APLICACAO,
...
function emSegundoPlano(promessa) {
	const segura = promessa.catch((erro) => console.error("[whatsapp-inbound] resposta_falhou", { erro: erro instanceof Error ? erro.message : String(erro) }));
...
Deno.serve(async (requisicao) => {
	...
	if (resposta.depois) emSegundoPlano(resposta.depois((contaId, conversaId) => responderConversa({
		contaId,
		conversaId
	}, porta)));
