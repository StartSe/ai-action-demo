...
// supabase/functions/tool-dnc/index.ts. Não edite à mão: rode `npm run pacote`.
//
...
function origemDoMotivo(reason) {
	return texto(reason).toLowerCase() === "wrong_number" ? "wrong_number" : "lead_request";
}
function notasDoPedido(entrada) {
	const reason = texto(entrada.reason);
	const partes = [["lead_request", "wrong_number"].includes(reason.toLowerCase()) ? "" : reason, texto(entrada.notes)].filter((parte) => parte !== "");
	return partes.length === 0 ? null : [...new Set(partes)].join(" | ");
}
function numeroDoInterlocutor(chamada, numeros) {
	const numero = texto(chamada.direction === "inbound" ? numeros.de : numeros.para) || texto(numeros.doLead);
	return numero === "" ? null : numero;
}
function despedida(origem) {
	const falas = origem === "wrong_number" ? FALAS_DAS_REGRAS_TRAVADAS.pessoaErrada : FALAS_DAS_REGRAS_TRAVADAS.naoPerturbe;
	return falas[falas.length - 1];
}
function criarToolDnc(leitura) {
	return criarFerramenta({
		...
		executar: {
			async ler(contexto) {
				...
				const instante = await leitura.bloqueioVigente(contexto.contaId, telefone) ?? new Date(contexto.agora()).toISOString();
				return {
			...
			async efeitos(contexto, leitura) {
				...
				if (plano === void 0) return;
				const contextoDoItem = (blockedAt) => ({
					call_id: contexto.chamada.id,
					origem: plano.origem,
					recorte: plano.notas,
					blocked_at: blockedAt
				});
				const abrirItem = (blockedAt) => contexto.escrita.abrirItemNaFila({
...
//#endregion
//#region supabase/functions/tool-dnc/index.ts
...
const leitura = {
	...
	async bloqueioVigente(contaId, telefone) {
		const { data, error } = await servico.from("dnc_entries").select("created_at").eq("account_id", contaId).eq("phone_e164", telefone).is("removed_at", null).maybeSingle();
		if (error) throw new Error(error.message);
