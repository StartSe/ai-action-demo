...
export function AcessoMCP() {
  ...
  const configuracao = codigoNovo
    ? JSON.stringify(
        { mcpServers: { "voz-do-cliente": { url: endereco, headers: { Authorization: `Bearer ${codigoNovo}` } } } },
        null,
        2
      )
    : null;
  ...
  return (
    <section className="card p-6 max-md:p-5">
      ...
      <p className="text-muted text-sm mb-4 max-w-[640px]">
        Gere um código de acesso para que um assistente de IA (Claude, ChatGPT e outros) analise comentários de clientes diretamente pela conversa, sem precisar abrir o navegador.
      </p>
      ...
      <div className="grid grid-cols-2 gap-6 max-md:grid-cols-1 mt-6">
        <div>
          ...
          <ol className="text-[13px] text-muted list-decimal pl-5 space-y-1">
            ...
            <li>Salve e reinicie o Claude Desktop: a Voz do Cliente aparece na lista de conectores.</li>
          </ol>
        </div>
        <div>
          ...
          <ol className="text-[13px] text-muted list-decimal pl-5 space-y-1">
            ...
            <li>Salve: a Voz do Cliente aparece nas ferramentas disponíveis dentro da conversa.</li>
          </ol>
        </div>
      </div>
