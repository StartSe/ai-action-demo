// Ponte oficial com o ChatGPT pelo Codex App Server (@openai/codex fixado em 0.155.1), copiada do
// build-agentflows (mesmo desenho do radar-sinais): login por código de dispositivo, sessão privada em
// DATA_DIR/chatgpt (nunca herda credenciais da máquina), sem terminal/arquivos/navegador, e ferramentas
// dinâmicas (dynamicTools) para o agente do site. Próprio do app; lib/ai.ts (OpenRouter) não muda.
...
export class ChatGPTBridge {
  ...
  async beginLogin(): Promise<DeviceLogin> {
    await this.start();
    if (this.turns.size)
      throw new Error("Aguarde o agente terminar antes de trocar a conta.");
  ...
  async run({
    ...
    if (!(await this.account()).account)
      throw new Error("Conecte sua conta ChatGPT para criar e editar o site.");
    ...
    const r = await this.rpc<{ thread: { id: string } }>("thread/start", {
      ...
      environments: [],
      baseInstructions:
        "Você ajuda a construir e editar o site de uma empresa. Responda em português. Use somente as ferramentas explicitamente fornecidas. Não use terminal, arquivos, navegador ou outras capacidades do sistema.",
...
const globalChat = globalThis as typeof globalThis & {
  cloneSiteChatGPT?: ChatGPTBridge;
};
export function chatGPT() {
  return (globalChat.cloneSiteChatGPT ??= new ChatGPTBridge());
}
