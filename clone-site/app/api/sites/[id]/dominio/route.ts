...
import { cadastrarDominio, estadoDominio, hostDoServico, removerDominio, dominioRenderConectado } from "@/lib/render";
...
async function alvoCname(req: Request, servicoId?: string, urlServico?: string): Promise<string> {
  if (urlServico) { try { return new URL(urlServico).hostname; } catch { /* consulta o serviço */ } }
  const doServico = dominioRenderConectado(servicoId) ? await hostDoServico(servicoId) : null;
...
async function estado(req: Request, dominio?: string, servicoId?: string, urlServico?: string) {
  const conectado = dominioRenderConectado(servicoId);
  const base = { hospedagemConectada: conectado, alvoCname: await alvoCname(req, servicoId, urlServico) };
  ...
  try {
    const e = await estadoDominio(dominio, servicoId);
...
export async function GET(req: Request, { params }: RouteContext<"/api/sites/[id]/dominio">) {
  ...
  return Response.json(await estado(req, projeto.dominio, projeto.render?.siteId, projeto.render?.url));
}
...
export async function PUT(req: Request, { params }: RouteContext<"/api/sites/[id]/dominio">) {
  ...
  try {
    ...
    if (dominioRenderConectado(projeto.render?.siteId)) {
      try {
        await cadastrarDominio(projeto.dominio, projeto.render?.siteId);
      } catch (err) {
        aviso = err instanceof Error ? err.message : "Não foi possível cadastrar na hospedagem; faça o passo 2 à mão.";
      }
    }
    const e = await estado(req, projeto.dominio, projeto.render?.siteId, projeto.render?.url);
...
export async function DELETE(req: Request, { params }: RouteContext<"/api/sites/[id]/dominio">) {
  ...
  try {
    ...
    if (anterior.dominio && dominioRenderConectado(anterior.render?.siteId)) await removerDominio(anterior.dominio, anterior.render?.siteId);
    return Response.json({ ...(await estado(req, undefined, projeto.render?.siteId, projeto.render?.url)), projeto });
  } catch (err) {
    return respostaErroSites(err);
  }
}
