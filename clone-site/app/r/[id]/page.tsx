...
import { projetoDaPagina } from "@/lib/projetos";
...
export default async function Page({ params }: PageProps<"/r/[id]">) {
  ...
  // Página que pertence a um site: a entrega aponta para o workspace (/sites/[id]) em vez de "Publicar link".
  const projeto = projetoDaPagina(registro.id);

  return (
    <>
      <TopbarSite />
      <main className="max-w-[1100px] mx-auto px-8 pt-7 pb-12 max-md:px-4 max-md:pt-5 max-md:pb-10">
        <Resultado pagina={pagina} meta={registro.meta} id={registro.id} projetoId={projeto?.id} />
      </main>
    </>
  );
}
