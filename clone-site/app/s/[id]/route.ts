...
export async function GET(req: Request, { params }: RouteContext<"/s/[id]">) {
  const { id } = await params;
  const publicado = htmlPublicado(id);
  if (!publicado) return new Response(PAGINA_404, { status: 404, headers: cabecalhosSite() });
  // Só a página de um site conta como visita (assets e 404 não); sem esperar a gravação.
  if (publicado.projetoId) registrarVisita(publicado.projetoId, req);
