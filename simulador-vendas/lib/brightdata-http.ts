...
let sequencia = 0;
const MAX_RESPOSTA = 4 * 1024 * 1024;
...
async function lerResposta(r: Response, id: number): Promise<unknown> {
    ...
    try {
      ...
      try {
        for (;;) {
          ...
          if (total > MAX_RESPOSTA) throw new Error();
          partes.push(value);
        }
      } finally { await leitor.cancel().catch(() => {}); }
      corpo = JSON.parse(Buffer.concat(partes).toString("utf8"));
    } catch { throw new ErroMCP(502, "Resposta MCP inválida."); }
  ...
  try {
    while (true) {
      ...
      if (total > MAX_RESPOSTA) throw new ErroMCP(502, "Resposta MCP excedeu o limite.");
...
async function enviar(conexao: ConexaoMCP, method: string, params: Record<string, unknown>, sessao?: Sessao, notificacao = false): Promise<{ resultado: unknown; idSessao?: string }> {
  ...
  try {
    ...
    if (!r.ok) { await r.body?.cancel(); throw new ErroMCP(r.status, "O serviço recusou a consulta."); }
    if (notificacao) {
      await r.body?.cancel();
...
function iniciar(conexao: ConexaoMCP): Promise<Sessao> {
  ...
  const promessa = (async () => {
    const { resultado, idSessao } = await enviar(conexao, "initialize", {
      protocolVersion: PROTOCOLOS[0], capabilities: {}, clientInfo: { name: "simulador-vendas", version: "0.1.0" },
    });
