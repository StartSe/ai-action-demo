...
export function ConectarLivekit({ aoConectar, configurada, porAmbiente }: { aoConectar: () => void; configurada: boolean; porAmbiente: boolean }) {
  ...
  return <div className="mb-5 rounded-xl bg-bg p-4">
    {autorizacao ? <>
      ...
      <button type="button" className="btn-primary !w-auto" disabled={iniciando || porAmbiente} onClick={() => void conectar()}>{iniciando ? "Preparando conexão…" : configurada ? "Conectar outro projeto LiveKit" : "Conectar LiveKit Cloud"}</button>
    ...
    <p className="text-xs text-muted mt-3">Conexão experimental pelo fluxo da ferramenta oficial do LiveKit. Você também pode preencher os campos abaixo.</p>
  </div>;
}
